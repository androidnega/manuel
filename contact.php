...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  $name = cms_form_upper($_POST['name'] ?? '');
  $email = cms_form_upper($_POST['email'] ?? '');
  $subject = cms_form_upper($_POST['subject'] ?? '');
  $message = cms_form_upper($_POST['message'] ?? '');
        ...
        <div class="reveal reveal-right reveal-delay-1 flex flex-col rounded-2xl bg-white border border-line p-5 sm:p-6 lg:p-8 shadow-sleek-sm">
          ...
          <form class="mt-5 flex flex-col flex-grow w-full gap-3" method="post" action="<?= page_url('contact.php') ?>">
            <div class="grid sm:grid-cols-2 gap-3 w-full">
              <input class="w-full rounded-xl border border-line bg-cloud px-3 py-3 text-sm uppercase outline-none focus:border-blue focus:ring-2 focus:ring-blue/10" name="name" placeholder="Your name" value="<?= htmlspecialchars($_POST['name'] ?? '') ?>" required />
              <input class="w-full rounded-xl border border-line bg-cloud px-3 py-3 text-sm uppercase outline-none focus:border-blue focus:ring-2 focus:ring-blue/10" name="email" type="email" placeholder="Email" value="<?= htmlspecialchars($_POST['email'] ?? '') ?>" required />
            </div>
            <input class="w-full rounded-xl border border-line bg-cloud px-3 py-3 text-sm uppercase outline-none focus:border-blue focus:ring-2 focus:ring-blue/10" name="subject" placeholder="Subject" value="<?= htmlspecialchars($_POST['subject'] ?? '') ?>" />
            <textarea class="w-full flex-grow min-h-[140px] rounded-xl border border-line bg-cloud px-3 py-3 text-sm uppercase outline-none focus:border-blue focus:ring-2 focus:ring-blue/10 resize-y" name="message" placeholder="Tell me about the project" required><?= htmlspecialchars($_POST['message'] ?? '') ?></textarea>
